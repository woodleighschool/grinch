...
func (c *Client) FetchUsers(ctx context.Context) ([]User, error) {
	...
	selectFields := []string{"id", "displayName", "userPrincipalName"}
...
const groupMemberFetchConcurrency = 8
...
func (c *Client) fetchAllGroupMembers(ctx context.Context, groups []Group) (map[uuid.UUID][]uuid.UUID, error) {
	...
	g.SetLimit(groupMemberFetchConcurrency)
	...
	m := make(map[uuid.UUID][]uuid.UUID, len(groups))
