// Package policies provides policy domain logic.
package policies
...
// Attachment links a rule to a policy with an action and an optional CEL condition.
...
// TargetKind identifies the type of policy target.
...
// Target describes an entity a policy applies to.
type Target struct {
	ID       uuid.UUID  `json:"id"`
	PolicyID uuid.UUID  `json:"policy_id"`
	Kind     TargetKind `json:"kind"      validate:"required,oneof=all user group machine"`
	RefID    *uuid.UUID `json:"ref_id"`
}

// ListItem summarises a policy for list endpoints.
