...
function renderRow(event: BlockedEvent) {
  return (
    <tr key={event.id}>
      ...
      <td>
        <span className="badge danger">{event.blocked_reason || 'Blocked'}</span>
      </td>
    </tr>
  );
}
...
function DirectoryStats({ groups, users, totalScopes }: DirectoryStatsProps) {
  ...
  const filteredGroups = useMemo(() =>
    groups.filter(g => g.display_name.toLowerCase().includes(searchTerm.toLowerCase())).slice(0, 20),
    [groups, searchTerm]
  );

  const filteredUsers = useMemo(() =>
    users.filter(u =>
      (u.display_name || u.principal_name).toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="card">
      ...
      {activeTab === 'overview' && (
        <div className="empty-state">
          ...
          <p><strong>Tip:</strong> Start with groups for broader access control, then use individual users for exceptions.</p>
        </div>
      )}
      ...
      {(activeTab === 'groups' || activeTab === 'users') && (
        <>
          ...
          <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
            ...
            {searchTerm && (activeTab === 'groups' ? filteredGroups : filteredUsers).length === 0 && (
              <div className="empty-state">
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const scopeCounts = await Promise.all(safeApps.map(async (app) => {
