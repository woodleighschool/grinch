...
export async function listScopes(appId: string): Promise<ApplicationScope[]> {
  const res = await fetch(`/api/apps/${appId}/scopes`, { credentials: 'include' });
  return handleResponse<ApplicationScope[]>(res);
}

export async function createScope(appId: string, payload: { target_type: 'group' | 'user'; target_id: string; action: 'allow' | 'block'; }): Promise<ApplicationScope> {
...
export async function listGroupMemberships(): Promise<GroupMembership[]> {
  const res = await fetch('/api/groups/memberships', { credentials: 'include' });
...
export function subscribeBlockedEvents(onEvent: (event: BlockedEvent) => void): () => void {
  const source = new EventSource('/api/events/blocked/stream', { withCredentials: true });
...
export async function updateSAMLSettings(settings: SAMLSettings): Promise<SAMLSettings> {
  const res = await fetch('/api/settings/saml', {
    method: 'PUT',
    headers: {
